...
def gpioOut(bits, pins):
    for bit, pin in zip(bits, pins):
        if bit == 1:
            pin.value = 1
    sleep(2)
    for pin in pins:
...
#   valid face output.
def run_images(valid_output, validated_image_filename, graph,infer_image):
...
def main():
    ...
    nick_img = cv2.imread('nick.jpg')
    alex_img = cv2.imread('alex.jpg')
    christina_img = cv2.imread('christina.jpg')
    ray_img = cv2.imread('ray.jpg')
    phuc_img = cv2.imread('phuc.jpg')
    sanika_img = cv2.imread('sanika.jpg')
    liu_img = cv2.imread('liu.jpg')
    
    imgs= []
    imgs.append(nick_img)
    imgs.append(liu_img)
    #imgs.append(ray_img)
    #imgs.append(alex_img)
    #imgs.append(christina_img)
    #imgs.append(phuc_img)
    imgs.append(sanika_img)

    names = {0 : 'Nick', 1 : 'Professor Liu', 2 : 'Sanika', 3 : 'Christina', 4 : 'Phuc', 5 : 'Sanika'}
    toPins = [[0,0], [0,1], [1,0], [1,1]] 
    camera = PiCamera()
    ...
    valid_outputs = []
    for img in imgs:
        valid_outputs.append(run_inference(img, graph))
    window_name = 'FaceNet'    
    for frame in camera.capture_continuous(rawCapture, format = "bgr", use_video_port = True):
        input_image = rawCapture.array
        img_infer = run_inference(input_image, graph)        
        #results = [None] * len(imgs)
        cv2.imshow(window_name, input_image)
        for index, valid_output in enumerate(valid_outputs):
            if (face_match(valid_output, img_infer)):
                print("Matches", names[index])
                gpioOut(toPins[index+1], pins)                
                sleep(10)
