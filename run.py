...
def display_objects_in_gui(source_image, filtered_objects,play):
    ...
    if raw_key == ord('s'):
        ...
        cv2.imwrite(os.path.join('imgs/', filename), display_image)
    return raw_key, num_objects
...
def main():
    ...
    for frame in camera.capture_continuous(rawCapture, format = "bgr", use_video_port = True):
        ...
        if play == True:
            ...
            print(time.time() - t0)
        raw_key, num_objects = display_objects_in_gui(display_image, filtered_objs,True)
